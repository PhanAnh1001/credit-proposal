...
(Subgraph 2)        (Subgraph 3)
      ...
      assemble_report                     ← fan-in
       ...
       quality_review                     ← LLM-as-Judge
        /         \
      END      retry weakest section      ← self-correction (tối đa 1 lần)
```
...
```
score ≥ 7 hoặc retry_count ≥ 2 → END
...
|------|---------------------|-------|
| `extract_company_info` | Few-shot (1 ví dụ đầy đủ) | Đọc MD → LLM extract → Pydantic CompanyInfo |
| `analyze_sector` | Chain-of-Thought (5 bước) | Web search + LLM synthesis → section 2 |
| `analyze_financial` | Chain-of-Thought (5 bước) + 2-stage LLM parse | PDF extract → ratios → LLM analyze → section 3 |
| `assemble_report` | Template-based | Ghép 3 sections → MD + 2 DOCX |
| `quality_review` | LLM-as-Judge | Score 3 outputs (0-10) + generate quality_feedback |
